...
export const engine: Engine = async (job, onProgress) => {
  ...
  copy.set(bytes);
  const raw = String(job.options.filename ?? "").trim();
  const base = sanitizeFileBase(raw || "merged-document");
  return [{ name: `${base}.pdf`, blob: new Blob([copy.buffer], { type: "application/pdf" }), kind: "pdf" }];
};